...
def register_functions(dp: Dispatcher, _bot: BotConstructor):
    """Registers the async functions"""
    ...
    async def end_search(message: types.Message):
        ...
        if _bot.search_keyword != "":
            if _bot.settings[f"{user_id}"]["lang"] == "English":
                ...
                await message.answer(f"Η αναζήτηση για τον όρο '{_bot.search_keyword}' τερματίστηκε",
                                     reply_markup=markup)
        else:
            if _bot.settings[f"{user_id}"]["lang"] == "English":
